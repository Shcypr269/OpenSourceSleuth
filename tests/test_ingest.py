...
class TestCLIMain:
    ...
    def test_main_stats_command(self, tmp_path):
        ...
        with patch("src.ingest.DATA_DIR", tmp_path):
            with patch.object(sys, "argv", ["sourcesleuth-ingest", "stats"]):
                result = main()
                assert result == 0
    ...
    def test_main_clear_command(self, tmp_path):
        ...
        with patch("src.ingest.DATA_DIR", tmp_path):
            with patch.object(sys, "argv", ["sourcesleuth-ingest", "clear"]):
                result = main()
                assert result == 0
    ...
    def test_main_arxiv_command_missing_data(self, tmp_path):
        ...
        with patch("src.ingest.DATA_DIR", tmp_path):
            with patch.object(sys, "argv", ["sourcesleuth-ingest", "arxiv"]):
                result = main()
