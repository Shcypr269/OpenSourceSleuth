...
try:
    ...
    OCR_AVAILABLE = False
    logger.warning(
        "OCR dependencies not installed. Install with: pip install sourcesleuth[ocr]"
    )
...
try:
    ...
    logger.warning(
        "pdf2image not installed. Install with: pip install sourcesleuth[ocr]"
    )
...
def ocr_image(image_path: str | Path, language: str = "eng") -> OCRResult:
    ...
    logger.info(
        ...
        image_path.name, len(text), avg_confidence,
    )
...
def ocr_pdf(pdf_path: str | Path, language: str = "eng", dpi: int = 300) -> list[OCRResult]:
    ...
    for page_num, image in enumerate(images, start=1):
        ...
        logger.info(
            ...
            page_num, len(text), avg_confidence,
        )
    
    logger.info(
        "OCR complete for %s: %d pages processed",
        pdf_path.name, len(results),
    )
    ...
    try:
        ...
        if len(combined_text.strip()) > 100:
            logger.info(
                ...
                pdf_path.name, len(combined_text),
            )
